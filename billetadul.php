    ...
    if (!empty($num) && !empty($date) && !empty($pwd)) {
        ...
        AND YEAR(date_expiration)='$dat' AND num_carte='$num'");
            ...
            if ($row1<=0) {
                ...
                header('location:paiementretour.php');
                exit();
            } elseif ($row2<=0) {
...
');
    ...
    while ($row = $rec->fetch()) {
        ...
        ?>
                ...
                <ul>
<?php
$rec2 = $pdo->prepare('SELECT * FROM passager p , vol v , reservation r , client c, avion a WHERE p.num_res=r.num_res AND r.num_client=c.num_client AND r.num_res=? AND r.num_client=? AND  r.num_vol_retour=? and v.num_vol=r.num_vol_retour AND v.matricule_avion=a.matricule_avion
');
        $rec2->execute([$num_reservation, $num_client, $num_volR]);
        if ($rows = $rec2->fetch()) {?>
                    <table>
                        <tr>
